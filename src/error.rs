...
impl From<Error> for io::Error {
    /// Convert a `serde_partiql::Error` into an `io::Error`.
    ///
    ...
    /// enum MyError {
    ///     Io(io::Error),
    ///     Json(serde_partiql::Error),
    /// }
    ///
    /// impl From<serde_partiql::Error> for MyError {
    ///     fn from(err: serde_partiql::Error) -> MyError {
    ///         use serde_partiql::error::Category;
    ///         match err.classify() {
    ///             Category::Io => {
    ///                 MyError::Io(err.into())
    ///             }
